...
> 本节描述的是模块化 Server 迁移前的架构基线。当前 `cmd/server` 已经
> 通过 bootstrap 组装控制、接入、查询和运维平面；旧上传原型已移除。

实现前的 `cmd/server` 曾承担较多启动和路由职责，`internal/server/modules` 处于原型阶段。以下迁移建议保留为历史背景；当前可靠入口已经由 bootstrap 组装：
